...
2. 插入行时，应上帝视角填写所有列，禁止写成未知或者空值。
3. 单元格中，不要出现逗号，语义分割应使用/代替。
{{tableEditRules}}
# 增删改dataTable操作方法
你可以使用以下函数对表格的数据进行增删改操作。
## 1. 在某个表格中插入新行，使用insert函数：
insertRow(tableIndex:number, data:{[colIndex:number]:string|number})
例如：insert(0, {0: '2021-10-01', 1: '12:00', 2: '教室', 3: '悠悠'})
## 2. 在某个表格中删除行，使用delete函数：
deleteRow(tableIndex:number, rowIndex:number)
例如：deleteRow(0, 0)
## 3. 在某个表格中更新行，使用update函数：
...
const defaultTableData = [
    ...
    },
    {
        ...
        isertNode: '当出现表中没有的新角色时，应插入', updateNode: "当角色的身体出现持久性变化时，例如伤痕/当角色有新的爱好，职业，喜欢的事物时/当角色更换住所时/当角色提到重要信息时", deleteNode: ""
    },
    {
        tableName: '角色与<user>社交表格', tableIndex: 2, columns: ['角色名', '对<user>关系', '对<user>态度', '对<user>好感'], note: '思考如果有角色和<user>互动，应什么态度',
        isertNode: '当出现表中没有的新角色时，应插入', updateNode: "当角色和<user>的交互不再符合原有的记录时/当角色和<user>的关系改变时", deleteNode: ""
    },
    {
